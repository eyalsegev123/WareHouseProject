...
class Volunteer {
    public:
        ...
        bool isCollector();//return true if this volunteer is a collector - AUXILIARY METHOD
        ...
        virtual ~Volunteer();
        virtual void setFree();//the volunteer is free again

    protected:
        int completedOrderId; //Initialized to NO_ORDER if no order has been completed yet, //its the last order that the volunteer completed
...
class LimitedDriverVolunteer: public DriverVolunteer {
    ...
    public:
        ...
        void acceptOrder(const Order &order) override; // Reset distanceLeft to orderDistance and decrease ordersLeft
        string toString() const override;
